...
class Encoder(object):
    ...
    def __init__(self, dboxes, figsize):
        self.dboxes = dboxes(figsize=figsize, order="ltrb")
        self.dboxes_xywh = dboxes(figsize=figsize, order="xywh").unsqueeze(dim=0)
...
class DefaultBoxes(object):
    def __init__(self, fig_size, feat_size, steps, scales, aspect_ratios, \
        ...
        fk_starts = fig_size/np.array(starts)
        self.aspect_ratios = aspect_ratios

        self.dboxes = [torch.zeros(38,38,4,5),torch.zeros(19,19,6,5),torch.zeros(10,10,6,5),\
        torch.zeros(5,5,6,5),torch.zeros(3,3,4,5),torch.zeros(1,1,4,5)]
        ...
        for idx, sfeat in enumerate(self.feat_size):
            ...
            for wh_kind, (w, h) in enumerate(all_sizes):
                for i, j in itertools.product(range(sfeat), repeat=2):
                    count += 1
                    cx, cy = (j)/fk[idx]+(0.5)/fk_starts[idx], (i)/fk[idx]+(0.5)/fk_starts[idx]
                    self.dboxes[idx][i,j,wh_kind] = torch.tensor([cx, cy, w, h, count], dtype=torch.float)
                    
            
            
#         self.dboxes = torch.tensor(self.default_boxes, dtype=torch.float)
        for i in range(6):
            self.dboxes[i][:,:,:,:-1] = self.dboxes[i][:,:,:,:-1].clamp(min=0, max=1)

        # For IoU calculation
        self.dboxes_ltrb = dboxes_to_ltrb(self.dboxes)
    ...
    def __call__(self, figsize, order="ltrb"):
        if order == "ltrb": 
            return Change_by_size(self.dboxes_ltrb, figsize)
        if order == "xywh": 
            return Change_by_size(self.dboxes, figsize)
        
def dboxes_to_ltrb(default_boxes):
    dboxes_ltrb_all = []
    for dboxes in default_boxes:
        dboxes_ltrb = dboxes.clone()
        dboxes_ltrb[:, :,:, 0] = (dboxes[:, :,:,0] - 0.5 * dboxes[:, :,:,2]).clone()
        dboxes_ltrb[:, :,:, 1] = (dboxes[:, :,:,1] - 0.5 * dboxes[:, :,:,3]).clone()
        dboxes_ltrb[:, :,:, 2] = (dboxes[:, :,:,0] + 0.5 * dboxes[:, :,:,2]).clone()
        dboxes_ltrb[:, :,:, 3] = (dboxes[:, :,:,1] + 0.5 * dboxes[:, :,:,3]).clone()     
        dboxes_ltrb_all.append(dboxes_ltrb)
    return dboxes_ltrb_all

def Change_by_size(dboxes, figsize):
    sizes = Find_feat(figsize)
    all_box =[]
    for i, size in enumerate(sizes):
        dbox = dboxes[i]
        all_box.append(dbox[:size[0],:size[1]].reshape(-1,5))
    new_tensor_box = torch.cat(all_box, 0)

    return new_tensor_box[new_tensor_box[:,-1].argsort()][:,:-1]

def odd_divide(num):
    return (num[0]+1)//2, (num[1]+1)//2
...
def dboxes300_coco():
    ...
    starts = [8, 16, 32, 64, 192, 300]
    # use the scales here: https://github.com/amdegroot/ssd.pytorch/blob/master/data/config.py
    scales = [21, 45, 99, 153, 207, 261, 315]
    aspect_ratios = [[2], [2, 3], [2, 3], [2, 3], [2], [2]]
    dboxes = DefaultBoxes(figsize, feat_size, (steps,starts), scales, aspect_ratios)
    return dboxes

def decode_results(predictions, figsize):
    dboxes = dboxes300_coco()
    encoder = Encoder(dboxes,figsize)
